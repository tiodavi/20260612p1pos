...
# 【核心修正】改用 before_request 鉤子函數，確保在正式收到瀏覽請求（環境變數已就緒）時才執行初始化
@app.before_request
def initialize_app():
    # 使用 Flask 物件內建屬性檢查，確保應用程式生命週期內只執行一次
...
def index():
    ...
    <body class="p-4 md:p-8">
        ...
        <script>
            ...
            // 【修正點】使用 Jinja2 的 safe 與 tojson 傳遞後端變數，避免引號解析錯誤導致 JS 壞掉
